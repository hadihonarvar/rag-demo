...
async def get_product(product_id: UUID4, psql_db_session: Session = Depends(get_psql_db), qdrant_db_session: QdrantClient = Depends(get_qdrant_db)):
    log.info(f"Getting product {product_id}")
    product = await get_product(product_id, psql_db_session, qdrant_db_session)
    return {"message": "Product retrieved successfully"}

@router.post("/update_product")
async def update_product(product: ProductDTO, psql_db_session: Session = Depends(get_psql_db), qdrant_db_session: QdrantClient = Depends(get_qdrant_db)):
    log.info(f"Updating product {product.name}")
    product = await update_product(product, psql_db_session, qdrant_db_session)
    return {"message": "Product updated successfully"}

@router.delete("/delete_product/{product_id}")
async def delete_product(product_id: UUID4, psql_db_session: Session = Depends(get_psql_db), qdrant_db_session: QdrantClient = Depends(get_qdrant_db)):
    log.info(f"Deleting product {product_id}")
    await delete_product(product_id, psql_db_session, qdrant_db_session)
